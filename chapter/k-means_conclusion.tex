...
\begin{table}
	...
	\end{tabular}
\end{table}
总结来说，如果追求速度推荐使用K-M$\text{C}^2$、均匀不放回采样(2)、AFK-M$\text{C}^2$和Lightweight Coreset，如果追求聚类质量推荐使用$\epsilon$-Coreset和$k$-means\(\vert \vert\) seeding。% remark: verify argument on $\epsilon$-Coreset
